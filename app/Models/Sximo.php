...
class Sximo extends Model {
	...
	public static function getRows( $args )
	{
	   ...
	   $key = with(new static)->primaryKey;
		...
		$rows = array();
	    $result = \DB::connection(\Session::get('conn'))->select( self::querySelect() . self::queryWhere(). " 
				{$params} ". self::queryGroup() ." {$orderConditional}  {$limitConditional} ");
		
		if($key =='' ) { $key ='*'; } else { $key = $table.".".$key ; }	
		$total = \DB::connection(\Session::get('conn'))->select( self::querySelect() . self::queryWhere(). " 
...
//		$total = $res[0]->total;
	...
	public static function prevNext( $id ){
	   ...
	   $next = '';
		...
		$Qnext = \DB::connection(\Session::get('conn'))->select( 
			self::querySelect() . 
			self::queryWhere().
			" AND ".$table.".".$key." > '{$id}'  ". 
		...
		$Qprev  = \DB::connection(\Session::get('conn'))->select( 
			self::querySelect() . 
			self::queryWhere().
			" AND ".$table.".".$key." < '{$id}'". 
	...
	public static function getRow( $id )
	{
	   ...
	   $key = with(new static)->primaryKey;
		...
		$result = \DB::connection(\Session::get('conn'))->select( 
				self::querySelect() . 
				self::queryWhere().
				" AND ".$table.".".$key." = '{$id}' ". 
				self::queryGroup()
	   ...
	   $key = with(new static)->primaryKey;
	    if($id == NULL )
        {
			...
			if(isset($data['updated_at'])) $data['updated_at'] = date("Y-m-d H:i:s");	
			 $id = \DB::connection(\Session::get('conn'))->table( $table )->insertGetId($data);				
			...
			if(isset($data['updated_at'])) $data['updated_at'] = date("Y-m-d H:i:s");			
			 \DB::connection(\Session::get('conn'))->table($table)->where($key,$id)->update($data);    
        }    
        return $id;    
	}			
	...
	{	
		$row =  \DB::connection(\Session::get('conn'))->table('tb_module')->where('module_name', $id)->get();
		$data = array();
		foreach($row as $r)
		{
			$data['id']		= $r->module_id; 
    ...
    {   
        ...
        if(count($limit) >=3)
        {
            ...
            if(count($parent)>=2 )
            {
            	$row =  \DB::connection(\Session::get('conn'))->table($table)->where($parent[0],$parent[1])->get();
            	 $row =  \DB::connection(\Session::get('conn'))->select( "SELECT * FROM ".$table." ".$condition ." AND ".$parent[0]." = '".$parent[1]."'");
            } else  {
	           $row =  \DB::connection(\Session::get('conn'))->select( "SELECT * FROM ".$table." ".$condition);
            }        
        }else{
            ...
            if(count($parent)>=2 )
            {
	            ...
	            $row =  \DB::connection(\Session::get('conn'))->table($table)->get();
            }	           
        }
	...
	function validAccess( $id)
	{
		...
		$row = \DB::connection(\Session::get('conn'))->table('tb_groups_access')->where('module_id','=', $id)
	...
	{	  
        $columns = array();
	    foreach(\DB::connection(\Session::get('conn'))->select("SHOW COLUMNS FROM $table") as $column)
        {
           //print_r($column);
	...
	static function getTableList( $db ) 
	{
		...
		foreach(\DB::connection(\Session::get('conn'))->select("SHOW TABLES FROM {$db}") as $table)
        {
