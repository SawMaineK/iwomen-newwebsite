...
class SiteHelpers
{
	...
	public static function formShow( $type , $field , $required ,$option = array()){
		...
		switch($type)
		{
			default;
				...
				if(isset($option['prefix']) && $option['prefix'] !='' or isset($option['sufix']) && $option['sufix'] !='')
				{
					...
					if($option['prefix'] !='')
						...
						$form .= "	<input type='text' name='{$field}' id='{$field}' value='{{ \$row['{$field}'] }}' {$mandatory} {$attribute} class='form-control {$extend_class}' />";
			...
			case 'hidden';
				$form = "
						{!! Form::hidden('{$field}', \$row['{$field}']) !!}\n";
				break;

			case 'textarea';
				if($required !='0') { $mandatory = 'required'; }
				$form = "
						<textarea name='{$field}' rows='5' id='{$field}' class='form-control {$extend_class}' {$mandatory} {$attribute} >{{ \$row['{$field}'] }}</textarea>\n";
				break;

			case 'textarea_editor';
				if($required !='0') { $mandatory = 'required'; }
				$form = "
						<textarea name='{$field}' rows='5' id='editor' class='form-control editor {$extend_class}' {$mandatory}{$attribute} >{{ \$row['{$field}'] }}</textarea>\n";
				break;				
			...
			case 'select';
				...
				if($option['opt_type'] =='datalist')
				{
					...
					if(isset($option['select_multiple']) && $option['select_multiple'] ==1)
					{
						...
						$form  .= 	"
									<select name='{$field}[]' rows='5' {$mandatory} multiple  class='select2 ' > \n";
						$form  .= 	"
									<?php 
						...
						$form  .= 	"
									<select name='{$field}' rows='5' {$mandatory}  class='form-control select2 ' id='{$field}'  > ";
						$form  .= 	"
									<?php 
										foreach(\$".$field."_opt as \$key=>\$val)
										{
			...
			case 'file';
				...
				if(isset($option['image_multiple']) && $option['image_multiple'] ==1)
				{
					...
					$form .="
							style='width:150px !important;' {$attribute} />
						 	<div >
			...
			case 'radio';
				...
				for($i=0; $i<count($opt);$i++) 
				{
					...
					$form .="
							@if(\$row['".$field."'] == '".ltrim(rtrim($row[0]))."') checked=\"checked\" @endif";
					$form .= " > ".$row[1]." </label>";
				}
				break;
	...
	public static  function bulkForm( $field, $forms = array(), $value ='')
	{
		...
		switch($type)
		{
			...
			case 'select';
				...
				$form = "<select name='$field{$bulk}'  class='form-control select2' $mandatory >
							<option value=''> -- Select  -- </option>
							$opts
						</select>";
				break;	
			...
			$values = explode(',',$value);
				...
				foreach($values as $val)
				{
					if($val != '' && $format_value)
					{
						...
						$vals .= '<a href="'.url($val).'" target="_blank" class="previewImage"><img src="'.$val.'" border="0" width="30" class="img-circle" style="margin-right:2px;" /></a>';
					}
				}	 
		    $value = $vals;
