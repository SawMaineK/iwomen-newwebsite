...
class UserListController extends Controller {
	...
	public function getIndex( Request $request )
	{
		...
		return view('user.list.index',$this->data);
	}	
	...
	function getUpdate(Request $request, $id = null)
	{
		...
		$this->data['id'] = $id;
		return view('user.list.form',$this->data);
	}	
	...
	public function getShow( $id = null)
	{
		...
		$this->data['id'] = $id;
		$this->data['access']		= $this->access;
		return view('user.list.view',$this->data);	
	}	
	...
	function postSave( Request $request, $id =0)
	{
		...
		if ($validator->passes()) {
			...
			return Redirect::to('user/update/'.$id)->with('messagetext',\Lang::get('core.note_error'))->with('msgstatus','error')
	...
	function getBlast()
	{
		...
		return view('user.list.blast',$this->data);		
	}
			...
			if(!is_null($request->input('groups')))
			{
				...
				for($i=0; $i<count($groups); $i++)
				{
					...
					foreach($users as $row)
					{
						...
						{ 
							Mail::send('user.list.email', $data, function ($message) use ($data) {
					    		$message->to($data['to'])->subject($data['subject']);
					    	});


					    } else {
					    	$message = view('user.list.email',$data);
				...
				return Redirect::to('user/blast')->with('messagetext','Total '.$count.' Message has been sent')->with('msgstatus','success');

			}
			return Redirect::to('user/blast')->with('messagetext','No Message has been sent')->with('msgstatus','info');
			

		} else {

			return Redirect::to('user/blast')->with('messagetext', 'The following errors occurred')->with('msgstatus','error')
			->withErrors($validator)->withInput();
