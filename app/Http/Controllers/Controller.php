...
class Controller extends BaseController
{
		...
		foreach($str as $f){
			...
			if($f['view'] ==1) 
			{
				if($f['type'] =='textarea_editor' || $f['type'] =='textarea')
				{
					...
					{	
						...
						if($f['option']['upload_type'] =='file')
						{
							...
							if(!is_null($request->file($field)))
							{
								...
								if($f['option']['resize_width'] != '0' && $f['option']['resize_width'] !='')
								{
									...
									if(count($resizeW) > 0){
										...
										foreach ($resizeW as $key => $w) {
											if($key > 0){
												if( !isset($resizeH[$key]) || $resizeH[$key] == 0  )
												{
													$resizeH[$key] = $w;
												}
											 	$newFile = $destinationPath.'/x'.$w.'/';
											 	if(!is_dir($newFile)){
											 		mkdir($newFile, 0777, true);
											 	}
											 	$newFile .= $newfilename;
												\SiteHelpers::cropImage($w , $resizeH[$key] , $orgFile , $extension, $newFile);
												if($id != null){
										...
										\SiteHelpers::cropImage($resizeW[0] , $resizeH[0] , $orgFile , $extension, $orgFile);
										if($id != null){
											$obj = $this->model->find($id);
											if (strpos($obj[$field], 'http://') !== false){
												$url = explode('/', $obj[$field]);
								...
								if( $uploadSuccess ) {
									if(isset($f['option']['save_full_path']) && $f['option']['save_full_path'] == 1){
										$data[$field] = url('/').'/'.$destinationPath.'/'.$newfilename;
									}else{
										$data[$field] = $newfilename;
									}
					...
					// if post is auto val
					if($f['type'] =='auto_val'){
						if(isset($f['option']['val_type']) && $f['option']['val_type'] == 'num'){
							if(isset($f['option']['str_prefix']) && strlen($f['option']['prefix']) > 0)
								$data[$field] = $id == null ? $f['option']['str_prefix'].rand(10000,10000000) : $request->input($field);
							else
								$data[$field] = $id == null ? rand(10000,10000000) : $request->input($field);
						}else{
							if(isset($f['option']['str_prefix']) && strlen($f['option']['prefix']) > 0)
								$data[$field] = $id == null ? $f['option']['str_prefix'].str_random(10) : $request->input($field);
							else
								$data[$field] = $id == null ? str_random(10) : $request->input($field);
...

	{
		...
		$model->where($detail['key'],$id)->whereNotIn('id', $request['bulk_id'])->delete();
    ...
    function postMultisearch( Request $request)
    {
        ...
        return Redirect::to($this->module.'?search='.substr($items,0,strlen($items)-1).'&md='.$request->get('md'));
    }
    ...
    public function getSearch()
    {
        ...
        return view('.utility.search',$this->data);
    ...
    function getLookup( Request $request, $id)
    {
        ...
        if(count($args)>=2) 
        {
            ...
            $data['key']        = $info['key'];
            //$data['ids']      = 'md'-$info['id'];
            return view('.utility.masterdetail',$data);
