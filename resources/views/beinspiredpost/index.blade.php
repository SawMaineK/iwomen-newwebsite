        ...
        <div class="portlet light bordered animated fadeInUp ">
        	<div class="portlet-title">
                ...
                <div class="grid actions">
                    @if($access['is_add'] ==1)
                    <a href="{{ URL::to('beinspiredpost/update?return='.$return) }}" class="tips btn btn-xs btn-success" title="{{ Lang::get('core.btn_create') }}">
                    ...
                    @endif
                    <a href="{{ URL::to( 'beinspiredpost/search?return='.$return) }}" class="tips btn btn-xs btn-info" onclick="SximoModal(this.href,'Advance Search'); return false;" title="{{ Lang::get('core.btn_search') }}"><i class="fa  fa-search"></i> <span class="hidden-xs">{{ Lang::get('core.btn_search') }}</span></a>
                    @if($access['is_excel'] ==1)
                    <a href="{{ URL::to('beinspiredpost/download?return='.$return) }}" class="tips btn btn-xs btn-warning" title="{{ Lang::get('core.btn_download') }}">
            ...
            <div class="portlet-body">
                ...
                {!! Form::open(array('url'=>'beinspiredpost/delete/0?return='.$return, 'class'=>'form-horizontal' ,'id' =>'SximoTable' )) !!}
                                ...
                                <td>
                                    <div class="dropdown">
                                        ...
                                        <ul class="dropdown-menu">
                                            ...
                                            <li><a href="{{ URL::to('beinspiredpost/show/'.$row->id.'?return='.$return)}}" class="tips" title="{{ Lang::get('core.btn_view') }}"><i class="fa  fa-search "></i> {{ Lang::get('core.btn_view') }} </a></li>
                                            @endif @if($access['is_edit'] ==1)
                                            <li><a href="{{ URL::to('beinspiredpost/update/'.$row->id.'?return='.$return) }}" class="tips" title="{{ Lang::get('core.btn_edit') }}"><i class="fa fa-edit "></i> {{ Lang::get('core.btn_edit') }} </a></li>
                                            @endif
                                        </ul>
                                    </div>
                ...
                @else
                    <p class="text-center" style="padding:50px 0;">{{ Lang::get('core.norecord') }}
                        <br />
                        <br />
                        <a href="{{ URL::to('beinspiredpost/update?return='.$return) }}" class="btn btn-default "><i class="icon-plus-circle2"></i> New beinspiredpost </a>
                    </p>
                @endif
            </div>
        </div>
